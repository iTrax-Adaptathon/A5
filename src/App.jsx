...
function EditChallenge({ user }) {
  const { id } = useParams(); const navigate = useNavigate(); const [form, setForm] = useState(null); const [error, setError] = useState(''); const [busy, setBusy] = useState(false)
  useEffect(() => { supabase.from('challenges').select('*').eq('id', id).single().then(({ data, error: loadError }) => { if (loadError) setError(errorText(loadError)); else if (data.creator_id !== user.id) setError('Only the challenge creator can edit this challenge.'); else setForm({ title: data.title || '', description: data.description || '', category: data.category || 'Community', difficulty: data.difficulty || 'medium', start_date: data.start_date || '', end_date: data.end_date || '' }) }) }, [id, user.id])
  const set = key => event => setForm(old => ({ ...old, [key]: event.target.value }))
  async function submit(event) { event.preventDefault(); setBusy(true); setError(''); const { error: updateError } = await supabase.from('challenges').update(form).eq('id', id).eq('creator_id', user.id); setBusy(false); if (updateError) return setError(errorText(updateError)); navigate(`/challenge/${id}`) }
  if (!form) return <section className="form-page"><h1>{error || 'Loading challenge…'}</h1></section>
  return <section className="form-page"><p className="eyebrow">EDIT CHALLENGE</p><h1>Refine your challenge.</h1><form className="form-card" onSubmit={submit}><label>Challenge title<input required maxLength="120" value={form.title} onChange={set('title')} /></label><label>Description<textarea required value={form.description} onChange={set('description')} /></label><div className="two-col"><label>Category<select value={form.category} onChange={set('category')}>{categories.slice(1).map(x => <option key={x}>{x}</option>)}</select></label><label>Difficulty<select value={form.difficulty} onChange={set('difficulty')}>{difficulties.slice(1).map(x => <option key={x} value={x}>{x[0].toUpperCase() + x.slice(1)}</option>)}</select></label></div><div className="two-col"><label>Start date<input type="date" value={form.start_date} onChange={set('start_date')} /></label><label>End date<input required type="date" value={form.end_date} onChange={set('end_date')} /></label></div>{error && <p className="notice">{error}</p>}<div className="form-actions"><Link className="secondary" to={`/challenge/${id}`}>Cancel</Link><button className="primary" disabled={busy}>{busy ? 'Saving…' : 'Save changes'}</button></div></form></section>
}

function ChallengeDetail({ user }) {
  const { id } = useParams(); const navigate = useNavigate(); const [challenge, setChallenge] = useState(null); const [participants, setParticipants] = useState([]); const [isMember, setIsMember] = useState(false); const [message, setMessage] = useState(''); const [progress, setProgress] = useState(''); const [busy, setBusy] = useState(false)
...
function Leaderboard() {
  ...
  useEffect(() => { (async () => { let query = supabase.from('participants').select('id, user_id, progress, challenges!inner(id, title)').order('progress', { ascending: false }); if (challengeId) query = query.eq('challenge_id', challengeId); const { data, error } = await query; if (error) setMessage(errorText(error)); else { setRows(data || []); const ids = [...new Set((data || []).map(r => r.user_id))]; if (ids.length) { const profiles = await supabase.from('profiles').select('id, name').in('id', ids); setNames(Object.fromEntries((profiles.data || []).map(p => [p.id, p.name]))) } } setLoading(false) })() }, [challengeId])
  return <section><p className="eyebrow">LIVE PROGRESS</p><h1>{challengeId ? 'Challenge leaderboard' : 'Community leaderboard'}</h1><p className="muted">Ranked by current normalized completion progress. Final verified scores are calculated separately after review.</p>{message && <p className="notice">{message}</p>}<div className="leaderboard">{loading ? <p>Loading rankings…</p> : rows.length ? rows.map((row, index) => <article className="score-row" key={row.id}><span className="rank">#{index + 1}</span><div className="score-person"><strong>{names[row.user_id] || 'Community member'}</strong><small>{row.challenges?.title}</small></div><div className="progress leaderboard-progress"><i style={{ width: `${Math.min(100, row.progress || 0)}%` }} /></div><strong className="final-score">{row.progress || 0}%</strong></article>) : <div className="empty">No participant progress has been recorded yet.</div>}</div></section>
}
...
function App() {
  ...
  return <Shell session={session}><Routes><Route path="/" element={<Browse user={session.user} />} /><Route path="/auth" element={<Navigate to="/" replace />} /><Route path="/create" element={<CreateChallenge user={session.user} />} /><Route path="/challenge/:id" element={<ChallengeDetail user={session.user} />} /><Route path="/challenge/:id/edit" element={<EditChallenge user={session.user} />} /><Route path="/challenge/:id/submit" element={<SubmitResult user={session.user} />} /><Route path="/leaderboard" element={<Leaderboard />} /><Route path="/profile" element={<Profile user={session.user} />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes></Shell>
}
